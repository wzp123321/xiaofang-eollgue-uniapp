<template>
	<view class="search-container">
		...
		<view class="search-container-wrap">
			...
			<view class="search-container-wrap-content" v-if="dataList.length>0">
				<SearchTitle v-for="(item, idx) in dataList" :key="'search_item_' + idx" :item="item" :keyword="searchTxt" />
			</view>
			<no-data v-else style="padding-top:200rpx;padding-bottom: 40px;" />
		</view>
	</view>
</template>
...
<script>
	...
	import SearchTitle from '../search/components/search-title/index.vue'
	export default {
		onLoad(options) {
			// 如果存在规范no
			if (options.normsNo) {
