<template>
  <view :class="['vip-purchase', isIOS ? '' : 'flex-column-space']">
    <view class="vip-purchase-type">
      ...
      ></PayTypeChoose>
      <image
        class="vip-purchase-type-rights"
        src="../../../static/personal/vip-rights.png"
        mode=""
      ></image>
    </view>
      ...
      <view class="pay-operation flex-row-space">
        <label class="radio">
          <radio
            ...
            "
          /><text @tap="linkToServiceAgreement"
            >已阅读并同意《消防学院会员用户协议》</text
          >
        </label>
        <button type="default" @tap="handlePay">
          立即支付<text style="font-size: 11px"
            >￥{{ goodInfo.rebatePrice || goodInfo.price }}</text
          >
        </button>
      </view>
    </view>
...
import { handleAliPay, handleWeixinPay } from "../../../service/index.js";
import { payTypes } from "../config/index.js";
...
export default {
  ...
  methods: {
    ...
    async getPaymentInfo() {
      ...
      const { payType } = this;
      const { goodsId, rebateId, rebatePrice, price } = this.goodInfo;
      let orderInfo = "";
      try {
    ...
    getApplePay() {
      ...
      const { userInfo, iapChannel } = this;
      const { productid } = this.goodInfo;
      const that = this;
